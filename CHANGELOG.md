...
Heartbeat extend-drift fix (P1 liveness, fleet-wide — same bug in all four SDKs), redesigned as **lead-estimate** after adversarial review of the first (alternate-beat) cut.
...
- **Heartbeat no longer drifts the reservation expiry outward — and no longer lapses it either.** The protocol's `extend_by_ms` extends **relative to the reservation's current `expires_at_ms`**, not to request time (`cycles-protocol-v0.yaml`), but the heartbeat sent `ExtendRequest::new(ttl_ms)` on *every* `ttl/2` beat — drifting the expiry outward by `ttl/2` per beat. Consequences: a killed process left the reserved budget locked until the drifted expiry (a zombie window that grew with runtime, capped only by the server's `max_extensions` ≈ 10 → up to ~6×ttl at defaults), and extensions burned twice as fast as needed, so runs longer than ~`max_extensions × ttl/2` exhausted the allowance and lost heartbeat protection mid-flight.
  ...
  The fix is a **lead-estimate** scheduler (`src/heartbeat.rs`). Each `ttl/2` beat estimates how far the server-side expiry leads "now" — `lead = (known_expiry − initial_expiry) + ttl − elapsed`, where `known_expiry`/`initial_expiry` are server-frame ms (the reserve response's `expires_at_ms`, now threaded from the guard into the heartbeat, and each extend response's) and `elapsed` is client-monotonic ms. Server-frame minus server-frame, monotonic minus monotonic: the client wall clock is never compared against the server's, so clock skew cannot corrupt the estimate, and the estimate is conservative by construction (extends early, never late). A beat extends by `ttl_ms` when `lead < 1.5·ttl` and skips otherwise. Net: no drift (lead stays bounded), no lapse (any lead shortfall — failures, clamped grants, small TTLs — triggers an extend on the very next beat), and steady-state extension consumption is still halved.
  ...
  - **Beat slip by one RTT per cycle:** sleeping after each awaited extend pushed every beat later by the request's round trip. Beats now come from `tokio::time::interval_at` (missed-tick behavior `Skip`), anchoring the cadence; the lead math uses the tick's scheduled instant.
  ...
  Spec-review follow-up (same release): **tenant policy `max_reservation_ttl_ms` silently caps the granted TTL** at reserve (governance default 1 hour), and the create response has no effective-TTL field — so seeding the heartbeat from the *requested* TTL schedules the first beat far too late (a 24 h request capped to 1 h → first beat at 12 h, 11 h after expiry). The client now recovers the **effective TTL** from two server-frame samples: the reserve response's `expires_at_ms` and its HTTP `Date` header (new `ApiResponse::date_ms`, parsed via `httpdate`, now a direct dependency — already in the tree via hyper) — `clamp(expires_at_ms − date_ms, 1000 ms, requested)`, falling back to the requested TTL when either sample is missing or the `Date` is unparseable. Server-frame minus server-frame, so this too is clock-skew-free, and the `Date` header's 1-second resolution is negligible against the spec's 1000 ms TTL minimum. The effective TTL drives *everything*: the beat interval (`effective/2`), the lead formula's `ttl` term, the `1.5·ttl` skip threshold, and the per-beat extend amount. The permanent stop set also gained `TENANT_CLOSED` (tenant closure is irreversible without administrative action) and `NOT_FOUND` / raw HTTP 404 (a 404'd reservation never returns).

  Cancellation semantics are unchanged (`CancellationToken`; the guard cancels on commit/release/drop). Regression tests (`tests/heartbeat_test.rs`, wiremock with dynamic `expires_at_ms` responders and a `Date`-stamped reserve mock) pin: the v2 cadence extend@1 / extend@2 / skip@3 / extend@4 under full grants; the capped scenario (requested 8000 / granted 2000 → 1000 ms beats extending by 2000, the full cadence derived from the effective TTL); garbage `Date` falling back to the requested TTL; same-key retry after a 500 then fresh key after success (asserted from received request bodies); permanent-failure stop for 409 `MAX_EXTENSIONS_EXCEEDED`, 409 `TENANT_CLOSED`, and 404 `NOT_FOUND` (no further requests); ttl 1200 staying alive across four 600 ms beats (no floor); clamped grants (+ttl/4) extending every beat; and a 200 with an unknown status counting as applied (beat 3 skips — pinning the reversal). Lead-math, effective-TTL, and permanent-classification unit tests live in `src/heartbeat.rs`; `Date`-parsing unit tests in `src/response.rs`.
